...
<script setup lang="ts">
  ...
  import { mkh, useProfileStore, PageDefinition, useMessage } from 'mkh-ui'
  ...
  const allButtons: Ref<any[]> = ref([]) //保存所有的按钮对象引用，方便全选时切换
  ...
  //刷新菜单树
  const refreshTree = () => {
    menuApi.getTree({ groupId: role.value.menuGroupId }).then((data: any) => {
      allButtons.value = []
  ...
  const refreshBindMenus = () => {
    roleApi.queryBindMenus({ id: role.value.id }).then((data: any) => {
      nextTick(() => {
      ...
      if (page) {
        ...
        if (page.buttons) {
          node.buttons = Object.values(page.buttons).map((m) => {
            return { ...m, menuId: node.item.id, checked: false }
          })
  ...
  const submit = () => {
    ...
    roleApi.updateBindMenus(model).then(() => {
      ...
      if (role.value.id === profileStore.roleId) {
