...
namespace Data.Adapter.Sqlite.Test
{
    public class GroupingTests : BaseTest
    {
        ...
        public void ToFirstTest1()
        {
            ...
            Assert.Equal("SELECT SUM([Id]) AS [Sum],[Title] AS [Title] FROM [Article] WHERE [Deleted] = 0 GROUP BY [Title], [Deleted] LIMIT 1 OFFSET 0;", sql);
        }
        ...
        public void ToFirstTest2()
        {
            ...
            var sql = query.ToFirstSql();
            Assert.Equal("SELECT SUM([Id]) AS [Sum] FROM [Article] WHERE [Deleted] = 0 GROUP BY [Deleted] HAVING SUM([Id]) > @P1 LIMIT 1 OFFSET 0;", sql);

            sql = query.ToFirstSqlNotUseParameters();
            Assert.Equal("SELECT SUM([Id]) AS [Sum] FROM [Article] WHERE [Deleted] = 0 GROUP BY [Deleted] HAVING SUM([Id]) > 3 LIMIT 1 OFFSET 0;", sql);
        }
    }
}
