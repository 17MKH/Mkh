...
namespace Data.Adapter.SqlServer.Test
{
    public class RepositoryTest : BaseTest
    {
        ...
        public void FirstTest()
        {
            ...
            Assert.Equal(Select + " WHERE [Title] = @P1 AND [Deleted] = 0 ORDER BY [Id] ASC OFFSET 0 ROW FETCH NEXT 1 ROW ONLY", sql);
        }
        ...
        public async void FunctionTest()
        {
            ...
            var avg = await _articleRepository.Find(m => m.Id > 5 && m.Id < 10).ToAvg(m => m.Price);

            Assert.Equal(1.5m, avg);
