...
namespace Data.Adapter.MySql.Test
{
    ...
    public class MultiTableQueryableSqlBuilderTests : DbContextTests
    {
        ...
        public void ResolveSortForLengthTest()
        {
            ...
            _queryBody.SetSort(exp, SortType.Asc);
            Assert.Equal(" ORDER BY CHAR_LENGTH(T1.`Title`) ASC", _builder.ResolveSort());
        }
        ...
        public void ResolveSortForDatetimeFormatTest()
        {
            ...
            _queryBody.SetSort(exp, SortType.Asc);
            Assert.Equal(" ORDER BY DATE_FORMAT(T1.`CreatedTime`,'%Y-%m-%d') ASC", _builder.ResolveSort());
        }
        ...
        public void ResolveSortForNewTest1()
        {
            ...
            Assert.Equal(" ORDER BY CHAR_LENGTH(T1.`Title`) DESC, SUBSTR(T2.`Name`,3,2) DESC", _builder.ResolveSort());
        }
